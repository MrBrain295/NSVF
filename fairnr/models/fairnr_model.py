...
class BaseModel(BaseFairseqModel):
    ...
    def add_args(cls, parser):
        ...
        parser.add_argument(
            ...
            action='store_true',
            help=
            'if set, a second ray marching pass will be performed based on the first time probs.'
        )
        parser.add_argument(
            '--use-fine-model',
            action='store_true',
            help=
            'if set, we will simultaneously optimize two networks, a coarse field and a fine field.'
        )
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        ...
        if (self.field_fine is None) and \
            ...
            for fine_key in [
                    key for key in state_dict.keys() if "field_fine" in key
            ]:
                state_dict[fine_key.replace("field_fine",
                                            "field")] = state_dict[fine_key]
    ...
    def forward(self, ray_split=1, **kwargs):
        with with_torch_seed(
                self.unique_seed
        ):  # make sure different GPU sample different rays
        ...
        if ray_split == 1:
            ...
            results = [
                self._forward(ray_start, ray_dir[:, :, i:i + chunk_size],
                              **kwargs)
        ...
        self.cache = {
            w:
            results[w].detach() if isinstance(w, torch.Tensor) else results[w]
            for w in results
        }
        return results
    ...
    def _forward(self, ray_start, ray_dir, **kwargs):
        ...
        if hits.sum() > 0:
            ...
            if self.hierarchical:  # hierarchical sampling
                ...
                samples, all_results = self.raymarching(ray_start,
                                                        ray_dir,
                                                        intersection_outputs,
                                                        encoder_states,
                                                        fine=True)
        ...
        hits = hits.reshape(-1)
        all_results = self.postprocessing(ray_start0, ray_dir0, all_results,
                                          hits, (S, V, P))
    ...
    def merge_outputs(self, outputs):
        ...
        for key in outputs[0]:
            if isinstance(outputs[0][key],
                          torch.Tensor) and outputs[0][key].dim() > 2:
    ...
    def visualize(self, sample, output=None, shape=0, view=0, **kwargs):
        ...
        img_id = '{}_{}'.format(sample['shape'][shape], sample['view'][shape,
                                                                       view])
        ...
        images = {}
        images = self._visualize(images, sample, output,
                                 [img_id, shape, view, width, 'render'])
        images = self._visualize(images, sample, sample,
                                 [img_id, shape, view, width, 'target'])
        if 'coarse' in output:  # hierarchical sampling
            images = self._visualize(images, sample, output['coarse'],
                                     [img_id, shape, view, width, 'coarse'])
    ...
    def add_eval_scores(self,
        ...
        for s in range(predicts.size(0)):
            for v in range(predicts.size(1)):
                ...
                p = recover_image(predicts[s, v],
                                  width=width,
                                  min_val=float(self.args.min_color))
                t = recover_image(targets[s, v],
                                  width=width,
                                  min_val=float(self.args.min_color))
                ...
                if 'ssim' in scores:
                    ssims += [
                        skimage.metrics.structural_similarity(
                            pn, tn, multichannel=True, data_range=1)
                    ]
                if 'psnr' in scores:
                    psnrs += [
                        skimage.metrics.peak_signal_noise_ratio(pn,
                                                                tn,
                                                                data_range=1)
                    ]
                    ...
                    with torch.no_grad():
                        lpips += [
                            criterion.lpips(
                                ...
                                2 * t.unsqueeze(-1).permute(3, 2, 0, 1) -
                                1).item()
                        ]
                if 'depths' in sample:
                ...
                if outdir is not None:
                    ...
                    def imsave(filename, image):
                        imageio.imsave(os.path.join(outdir, filename),
                                       (image * 255).astype('uint8'))

                    figname = '-{:03d}_{:03d}.png'.format(
                        sample['id'][s], sample['view'][s, v])
                    imsave('output' + figname, pn)
                    imsave('target' + figname, tn)
                    imsave(
                        'normal' + figname,
                        recover_image(compute_normal_map(
                            ...
                            width=width),
                                      ...
                                      width=width).numpy())
                    if 'featn2' in output:
                        imsave('featn2' + figname,
                               output['featn2'][s, v].cpu().numpy())
                    if 'voxel' in output:
                        imsave('voxel' + figname,
                               output['voxel'][s, v].cpu().numpy())
