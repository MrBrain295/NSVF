...
class NSVFModel(NeRFModel):
    ...
    def add_args(cls, parser):
        ...
        parser.add_argument('--fine-num-sample-ratio',
                            type=float,
                            default=0,
                            help='raito of samples compared to the first pass')
        parser.add_argument(
            '--inverse-distance-coarse-sampling',
            type=str,
            choices=['none', 'camera', 'origin'],
            default='none',
    ...
    def intersecting(self, ray_start, ray_dir, encoder_states, **kwargs):
        ...
        if self.reader.no_sampling and self.training:  # sample points after ray-voxel intersection
            ...
            ray_start = ray_start[sampled_masks.expand_as(ray_start)].reshape(
                S, -1, 3)
        ...
        if getattr(self.args, "fixed_num_samples", 0) > 0:
            intersection_outputs[
                'steps'] = intersection_outputs['min_depth'].new_ones(
                    *intersection_outputs['min_depth'].size()[:-1],
                    1) * self.args.fixed_num_samples
        else:
            intersection_outputs['steps'] = dists.sum(
                -1) / self.encoder.step_size
    ...
    def raymarching(self,
                    ...
                    encoder_states,
                    fine=False):
        samples, all_results = super().raymarching(ray_start, ray_dir,
                                                   intersection_outputs,
                                                   encoder_states, fine)
    ...
    def prepare_hierarchical_sampling(self, intersection_outputs, samples,
                                      all_results):
        intersection_outputs = super().prepare_hierarchical_sampling(
            intersection_outputs, samples, all_results)
        if getattr(self.args, "fine_num_sample_ratio", 0) > 0:
            intersection_outputs[
                'steps'] = samples['sampled_point_voxel_idx'].ne(-1).sum(
    ...
    def add_other_logs(self, all_results):
        return {
            'voxs_log': item(self.encoder.voxel_size),
    ...
    def split_voxels(self):
        logger.info("half the global voxel size {:.4f} -> {:.4f}".format(
            self.encoder.voxel_size.item(),
            self.encoder.voxel_size.item() * .5))
    ...
    def reduce_stepsize(self):
        logger.info("reduce the raymarching step size {:.4f} -> {:.4f}".format(
            self.encoder.step_size.item(),
            self.encoder.step_size.item() * .5))
...
def base_architecture(args):
    ...
    args.raymarching_stepsize_ratio = getattr(args,
                                              "raymarching_stepsize_ratio",
                                              0.0)
    ...
    args.inputs_to_texture = getattr(args, "inputs_to_texture",
                                     "feat:0:256, ray:4")
    ...
    args.background_stop_gradient = getattr(args, "background_stop_gradient",
                                            False)
    args.background_depth = getattr(args, "background_depth", 5.0)

    # raymarcher
    args.discrete_regularization = getattr(args, "discrete_regularization",
                                           False)
...
def nerf_style_architecture(args):
    ...
    args.feature_field_skip_connect = getattr(args,
                                              "feature_field_skip_connect", 3)
...
def nerf3_architecture(args):
    args.inputs_to_texture = getattr(args, "inputs_to_texture",
                                     "feat:0:256, pos:10, normal:4, ray:4")
    nerf2_architecture(args)
...
def nerf3nope_architecture(args):
    args.inputs_to_texture = getattr(args, "inputs_to_texture",
                                     "feat:0:256, pos:0:3, sigma:0:1, ray:4")
    nerf2_architecture(args)
...
def nerfold_architecture(args):
    ...
    args.feature_field_skip_connect = getattr(args,
                                              "feature_field_skip_connect", 3)
    args.no_layernorm_mlp = getattr(args, "no_layernorm_mlp", True)
    args.inputs_to_texture = getattr(
        args, "inputs_to_texture", "feat:0:256, normal:0:3, sigma:0:1, ray:4")
    nerf2_architecture(args)
...
def nerf2nope_architecture(args):
    args.inputs_to_texture = getattr(
        ...
        "feat:0:256, pos:0:3, normal:0:3, sigma:0:1, ray:4")
    nerf2_architecture(args)
...
def mdisco_nsvf_architecture(args):
    args.inputs_to_density = getattr(args, "inputs_to_density",
                                     "pos:10, context:0:256")
    args.inputs_to_texture = getattr(
        args, "inputs_to_texture",
        "feat:0:256, pos:10, normal:4, ray:4, context:0:256")
