...
class NSVFBGModel(NSVFModel):
    ...
    def add_args(cls, parser):
        ...
        parser.add_argument('--nerf-steps',
                            type=int,
                            help='additional nerf steps')
        parser.add_argument('--bg-field-args',
                            type=str,
                            default=None,
                            help='override args for bg field')
...
class SharedNSVFBGModel(NSVFBGModel):
    ...
    def postprocessing(self, ray_start, ray_dir, all_results, hits, sizes):
        ...
        self.bg_encoder.precompute(
            context=self.encoder.contexts(self.encoder.cid).unsqueeze(0))
        return super().postprocessing(ray_start, ray_dir, all_results, hits,
                                      sizes)
