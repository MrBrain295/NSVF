...
class ImageReader(Reader):
    ...
    @staticmethod
    def add_args(parser):
        parser.add_argument('--pixel-per-view',
                            type=float,
                            metavar='N',
        ...
        parser.add_argument(
            ...
            type=float,
            help=
            "this value determined the probability of sampling rays on masks")
        parser.add_argument(
            "--sampling-at-center",
            type=float,
            help=
            "only useful for training where we restrict sampling at center of the image"
        )
        parser.add_argument("--sampling-on-bbox",
                            action='store_true',
        ...
        parser.add_argument("--sampling-skipping-size",
                            type=int,
                            help="sample pixels if we have skipped pixels")
        parser.add_argument("--no-sampling-at-reader",
                            action='store_true',
        ...
        parser.add_argument(
            ...
            action='store_true',
            help=
            "if set, we assume extrinsics are trainable. We use 6D representations for rotation"
        )
    ...
    def forward(self, uv, intrinsics, extrinsics, size, path=None, **kwargs):
        ...
        for s in range(S):
            for v in range(V):
                ixt = intrinsics[s] if intrinsics.dim() == 3 else intrinsics[s,
                                                                             v]
                ext = extrinsics[s, v]
                ...
                if (self.deltas is not None) and (path is not None):
                    ...
                    ext = torch.cat([
                        torch.cat([rotation, translation[:, None]], 1), ext[3:]
                    ], 0)
                ray_start[s] += [translation]
                ray_dir[s] += [
                    get_ray_direction(translation, flatten_uv[s, v], ixt, ext,
                                      1)
                ]
    ...
    def sample_pixels(self, uv, size, alpha=None, mask=None, **kwargs):
        ...
        if self.args.sampling_on_mask > 0.0:
            probs = self.args.sampling_on_mask * probs + (
                1 - self.args.sampling_on_mask) * 1.0 / (H * W)
        ...
        sampled_index = sampling_without_replacement(
            torch.log(flatten_probs + TINY), num_pixels)
        sampled_masks = torch.zeros_like(flatten_probs).scatter_(
            -1, sampled_index, 1).reshape(S, V, H // C, W // C)

        if C > 1:
            sampled_masks = sampled_masks[:, :, :, None, :, None].repeat(
                1, 1, 1, patch_size, 1,
                patch_size).reshape(S, V, H // skip_size, W // skip_size)
            if skip_size > 1:
                full_datamask = sampled_masks.new_zeros(
                    S, V, skip_size * skip_size, H // skip_size,
                    W // skip_size)
                full_index = torch.randint(skip_size * skip_size, (S, V))
                for i in range(S):
                    for j in range(V):
                        full_datamask[i, j,
                                      full_index[i, j]] = sampled_masks[i, j]
                sampled_masks = full_datamask.reshape(
                    S, V, skip_size, skip_size, H // skip_size,
                    W // skip_size).permute(0, 1, 4, 2, 5,
                                            3).reshape(S, V, H, W)
