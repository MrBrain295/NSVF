...
class ShapeDataset(FairseqDataset):
    ...
    def __init__(self,
        ...
        if self.subsample_valid > -1:
            for key in _data_per_shape:
                _data_per_shape[key] = _data_per_shape[key][::self.
                                                            subsample_valid]
    ...
    def _load_shape(self, packed_data):
        ...
        if packed_data.get(
                'glb', None) is not None:  # additional global feature (if any)
            shape_data['global_index'] = np.loadtxt(
                packed_data['glb']).astype('int64')
        return shape_data
...
class ShapeViewDataset(ShapeDataset):
    ...
    def __init__(self,
        ...
        self.apply_mask_color = (self.bg_color[0] >= -1) & (
            self.bg_color[0] <= 1)  # if need to apply
        ...
        for r in range(repeat):
            ...
            for id in range(self.total_num_shape):
                ...
                perm_ids = np.random.permutation(
                    total_num_view) if train else np.arange(total_num_view)
                for key in _data_per_view:
                    element[key] = [
                        _data_per_view[key][id][i] for i in perm_ids
                    ]
                ...
                if r == 0 and preload:
                    ...
                    if binarize:
                        cache = self._load_binary(id,
                                                  np.arange(total_num_view),
                                                  phase_name)
                    else:
                        cache = self._load_batch(self.data, id,
                                                 np.arange(total_num_view))
                    self.cache += [cache]
                _index += 1
        ...
        for i, d in enumerate(self.data):
            if self.train:
                ...
                index_list = []
                for j in range(copy_id * num_view,
                               copy_id * num_view + num_view):
    ...
    def find_rgb(self):
        try:
            return self.select(
                [sorted(glob.glob(path + '/rgb/*.*g')) for path in self.paths])
        except FileNotFoundError:
            try:
                return self.select([
                    sorted(glob.glob(path + '/color/*.*g'))
                    for path in self.paths
                ])
            except FileNotFoundError:
    ...
    def find_depth(self):
        try:
            return self.select([
                sorted(glob.glob(path + '/depth/*.exr')) for path in self.paths
            ])
        except FileNotFoundError:
    ...
    def find_mask(self):
        try:
            return self.select(
                [sorted(glob.glob(path + '/mask/*')) for path in self.paths])
        except FileNotFoundError:
    ...
    def find_extrinsics(self):
        try:
            return self.select([
                sorted(glob.glob(path + '/extrinsic/*.txt'))
                for path in self.paths
            ])
        except FileNotFoundError:
            try:
                ...
                return self.select([
                    sorted(glob.glob(path + '/pose/*.txt'))
                    for path in self.paths
                ])
            except FileNotFoundError:
                raise FileNotFoundError(
                    'world2camera or camera2world matrices not found.')

    def find_intrinsics_per_view(self):
        try:
            return self.select([
                sorted(glob.glob(path + '/intrinsic/*.txt'))
                for path in self.paths
            ])
        except FileNotFoundError:
            return None
    ...
    def summary_view_data(self, _data_per_view):
        ...
        for k in range(num_of_objects):
            assert len(set([len(_data_per_view[key][k]) for key in keys
                            ])) == 1, "numer of views must be consistent."
        return [
            list(range(len(_data_per_view[keys[0]][k])))
            for k in range(num_of_objects)
        ]
    ...
    def _load_view(self, packed_data, view_idx):
        image, uv, ratio = data_utils.load_rgb(packed_data['rgb'][view_idx],
        ...
        extrinsics = geometry.parse_extrinsics(extrinsics,
                                               self.world2camera).astype(
                                                   'float32')  # this is C2W
        ...
        if packed_data.get('dep', None) is not None:
            z = data_utils.load_depth(packed_data['dep'][view_idx],
                                      resolution=self.resolution)
        if packed_data.get('mask', None) is not None:
            mask = data_utils.load_mask(packed_data['mask'][view_idx],
                                        resolution=self.resolution)
            if self.apply_mask_color:  # we can also not apply mask
                rgb = rgb * mask[None, :, :] + (1 - mask[
                    None, :, :]) * np.asarray(self.bg_color)[:, None, None]
        ...
        return {
            ...
            intrinsics,
            'depths':
            z.reshape(-1) if z is not None else None,
            'mask':
            mask.reshape(-1) if mask is not None else None,
            'size':
            np.array([rgb.shape[1], rgb.shape[2]] + ratio, dtype=np.float32)
        }

    def _load_batch(self, data, index, view_ids=None):
        if view_ids is None:
            view_ids = [
                next(self.data_index[index]) for _ in range(self.num_view)
            ]
        return index, self._load_shape(data[index]), [
            self._load_view(data[index], view_id) for view_id in view_ids
        ]
...
class ShapeViewStreamDataset(BaseWrapperDataset):
    ...
    ** HACK **: an alternative of the ShapeViewDataset
    ...
    def _load_batch(self, data, shape_id, view_ids):
        return shape_id, self.dataset._load_shape(data[shape_id]), [
            self.dataset._load_view(data[shape_id], view_id)
    ...
    def _load_binary(self, id, views, phase='train'):
        ...
        try:
            ...
            caches = [
                self._load_batch(self.data, id, view_id) for view_id in views
            ]
            cache = [
                caches[0][0], caches[0][1],
                [caches[i][2][0] for i in range(len(views))]
            ]
    ...
    def __getitem__(self, index):
        ...
        sample_index = [
            data_utils.sample_pixel_from_image(
                data['alpha'].shape[-1],
                self.num_sample,
                data.get('mask', None) if data.get(
                    'mask', None) is not None else data.get('alpha', None),
                self.sampling_on_mask,
                self.sampling_on_bbox,
                self.sampling_at_center,
                width=int(data['size'][1]),
                patch_size=self.patch_size) for data in data_per_view
        ]
...
class WorldCoordDataset(BaseWrapperDataset):
    ...
    def __getitem__(self, index):
        ...
        def camera2world(data):
            ...
            ray_dir = geometry.get_ray_direction(ray_start, data['uv'],
                                                 intrinsics, inv_RT, 1)
        ...
        return index, data_per_shape, [
            camera2world(data) for data in data_per_view
        ]
