...
class NeuralRenderer(object):
    ...
    def __init__(self,
        ...
        if test_camera_poses is not None:
            if os.path.isdir(test_camera_poses):
                self.test_poses = [
                    np.loadtxt(f)[None, :, :]
                    for f in sorted(glob.glob(test_camera_poses + "/*.txt"))
                ]
                ...
                if self.test_poses.shape[1] == 17:
                    self.test_frameids = self.test_poses[:,
                                                         -1].astype(np.int32)
            ...
            if test_camera_views is not None:
                ...
                self.test_poses = np.stack(
                    [self.test_poses[r] for r in render_views])
    ...
    def generate_rays(self,
        ...
        if inv_RT is None:
            ...
            cam_rot = geometry.look_at_rotation(cam_pos,
                                                at=self.at,
                                                up=self.up,
                                                inverse=True,
                                                cv=True)
        ...
        if self.test_int is not None:
            ...
            uv = torch.from_numpy(get_uv(h * rh, w * rw, h,
                                         w)[0]).type_as(intrinsics)
    ...
    def generate(self, models, sample, **kwargs):
        ...
        for shape in range(sample['shape'].size(0)):
            ...
            while step < max_step:
                ...
                uv, inv_RT = zip(*[
                    self.generate_rays(
                        k, sample['intrinsics'][shape], sample['size'][
                            shape, 0], self.test_poses[k] if self.
                        test_poses is not None else None)
                    for k in range(step, next_step)
                ])
                if self.test_frameids is not None:
                    assert next_step - step == 1
                    ids = torch.tensor(
                        self.test_frameids[step:next_step]).type_as(
                            sample['id'])
                else:
                    ids = sample['id'][shape:shape + 1]

                real_images = sample[
                    'full_rgb'] if 'full_rgb' in sample else sample['colors']
                real_images = real_images.transpose(
                    2, 3) if real_images.size(-1) != 3 else real_images
                ...
                _sample = {
                    ...
                    'colors':
                    torch.cat([
                        real_images[shape:shape + 1]
                        for _ in range(step, next_step)
                    ], 1),
                    'intrinsics':
                    sample['intrinsics'][shape:shape + 1],
                    'extrinsics':
                    torch.stack(inv_RT, 0).unsqueeze(0),
                    'uv':
                    torch.stack(uv, 0).unsqueeze(0),
                    'shape':
                    sample['shape'][shape:shape + 1],
                    'view':
                    torch.arange(step,
                                 next_step,
                ...
                with data_utils.GPUTimer() as timer:
                    outs = model(**_sample)
                logger.info("rendering frame={}\ttotal time={:.4f}".format(
                    step, timer.sum))
                    ...
                    for key in images:
                        ...
                        if type in self.output_type:
                            ...
                            if type == 'point':
                                ...
                                save_image(image,
                                           os.path.join(
                                               prefix, image_name + '.png'),
                                           format=None)
                                image_names.append(
                                    os.path.join(prefix, image_name + '.png'))

                    # save pose matrix
                    prefix = os.path.join(output_path, 'pose')
                    Path(prefix).mkdir(parents=True, exist_ok=True)
                    pose = self.test_poses[
                        k] if self.test_poses is not None else inv_RT[
                            k - step].cpu().numpy()
    ...
    def save_images(self, output_files, steps=None, combine_output=True):
        ...
        if not combine_output:
            for type in self.output_type:
                images = [
                    imageio.imread(file_path) for file_path in output_files
                    if type in file_path
                ]
                # imageio.mimsave('{}/{}_{}.gif'.format(self.output_dir, type, timestamp), images, fps=self.fps)
                imageio.mimwrite('{}/{}_{}.mp4'.format(self.output_dir, type,
                                                       timestamp),
                                 images,
                                 fps=self.fps,
                                 quality=8)
            ...
            images = [[
                imageio.imread(file_path) for file_path in output_files
                if type == file_path.split('/')[-2]
            ] for type in self.output_type]
            images = [
                np.concatenate([images[j][i] for j in range(len(images))], 1)
                for i in range(len(images[0]))
            ]
            imageio.mimwrite('{}/{}_{}.mp4'.format(self.output_dir, 'full',
                                                   timestamp),
                             images,
                             fps=self.fps,
                             quality=8)
    ...
    def merge_videos(self, timestamps):
        ...
        timestamp = time.strftime('%Y-%m-%d.%H-%M-%S',
                                  time.localtime(time.time()))
        writer = imageio.get_writer(os.path.join(self.output_dir,
                                                 'full_' + timestamp + '.mp4'),
                                    fps=self.fps)
        for timestamp in timestamps:
            tempfile = os.path.join(self.output_dir,
                                    'full_' + timestamp + '.mp4')
