...
class AABBRayIntersect(Function):
    ...
    def forward(ctx, voxelsize, n_max, points, ray_start, ray_dir):
        ...
        G = min(2048,
                int(2 * 10**9 / points.numel()))  # HACK: avoid out-of-memory
...
class SparseVoxelOctreeRayIntersect(Function):
    ...
    def forward(ctx, voxelsize, n_max, points, children, ray_start, ray_dir):
        G = min(2048, int(
            2 * 10**9 /
            (points.numel() + children.numel())))  # HACK: avoid out-of-memory
        S, N = ray_start.shape[:2]
        K = int(np.ceil(N / G))
        H = K * G
        if H > N:
        ...
        inds, min_depth, max_depth = _ext.svo_intersect(
            ray_start.float(), ray_dir.float(), points.float(), children.int(),
            voxelsize, n_max)
...
class TriangleRayIntersect(Function):
    ...
    def forward(ctx, cagesize, blur_ratio, n_max, points, faces, ray_start,
                ray_dir):
        # HACK: speed-up ray-voxel intersection by batching...
        G = min(2048, int(2 * 10**9 /
                          (3 * faces.numel())))  # HACK: avoid out-of-memory
        S, N = ray_start.shape[:2]
        K = int(np.ceil(N / G))
        H = K * G
        if H > N:
        ...
        inds, depth, uv = _ext.triangle_intersect(ray_start.float(),
                                                  ray_dir.float(),
                                                  face_points.float(),
                                                  cagesize, blur_ratio, n_max)
...
class UniformRaySampling(Function):
    ...
    def forward(ctx,
        ...
        sampled_idx, sampled_depth, sampled_dists = _ext.uniform_ray_sampling(
            pts_idx, min_depth.float(), max_depth.float(), noise.float(),
            step_size, max_steps)
...
def _parallel_ray_sampling(MARCH_SIZE,
    ...
    sampled_depth = .5 * (sampled_depth[:, 1:] + sampled_depth[:, :-1]
                          )  # mid-points
    ...
    sampled_depth.masked_fill_(
        (max_ids.ne(min_ids)) | (sampled_depth > _max_depth[:, None]) |
        (sampled_dists == 0.0), MAX_DEPTH)
...
def parallel_ray_sampling(MARCH_SIZE,
    ...
    outputs = zip(*[
        _parallel_ray_sampling(MARCH_SIZE,
